...
The device configuration can be used to choose an appropriate **DataCollectionMode** when configuring data selection. The following is an example of a BACnet device configuration that contains one routed device:
...
**Note:** Specifying the same combination of **DeviceId**, **ObjectType**, **ObjectId**, and **PropertyIdentifier** for multiple data selection items can result in data not being written to one or more streams. It is recommended that the combination of those properties be unique for each data selection item.
