...
In data source discovery, device information for all discovered devices is retrieved by the BACnet adapter, which is subsequently available through the following REST endpoint using any configuration tool that can execute an HTTP GET command:<br>`http://localhost:5590/api/v1/configuration/<ComponentId>/DeviceConfiguration/`.
...
| **DataCollectionMode** | Optional | `string` | Specifies the mode of data collection for the item. Must be one of `Poll`, `SubscribeCov`, or `SubscribeCovProperty`. Default value is `Poll`.<br><br>For more information, see [Change Of Value (COV) configuration](xref:BACnetCOVConfiguration) and [Polled data stream configuration](xref:BACnetPolledDataStreamConfiguration).|
...
| **ScheduleId** | Optional | `string` | Specifies a schedule ID to which the data selection item is linked. This data item is collected on the scheduled interval if Selected is set to `true`.<br>For **DataCollectionMode**=`Poll`, this is the interval at which this property is requested from the device.<br>For **DataCollectionMode**=`SubscribeCov` or `SubscribeCovProperty`, this is the interval at which a re-subscription request is sent.<br><br> For more information on schedules, see [Schedules configuration](xref:SchedulesConfiguration).|
